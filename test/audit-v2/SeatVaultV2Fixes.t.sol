...
///
///   GREEN:  forge test --match-path test/audit-v2/SeatVaultV2Fixes.t.sol
///   RED (controlled mutation, gates off):  SEAT_GATES_OFF=true forge test --match-path test/audit-v2/SeatVaultV2Fixes.t.sol
///   X-1 has no switch: it is unconditional code; its RED is the old `test_X1_F3_*` exploit, which passed before.
contract SeatVaultV2FixesTest is SeatVaultV2Base {
    ...
    function test_fix_X1_saleKillsTheOffer_andTheStakerGetsEveryTokenBack() public {
        ...
        vm.prank(buyer);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV2.OwnerChanged.selector, AGENT, agentOp, buyer));
    ...
    function test_fix_X2_idleSeatExpires_everyTokenBack_slotAndBackingFreed() public {
        ...
        vm.prank(anyone);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV2.NotIdle.selector, AGENT));
        vault.expire(AGENT);
    ...
    function test_fix_X2_aSeatInUseDoesNotExpire() public {
        ...
        vm.prank(anyone);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV2.NotIdle.selector, AGENT)); // loan open
        vault.expire(AGENT);
        _repay(agentOp, loan);
        vm.prank(anyone);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV2.NotIdle.selector, AGENT)); // idle for less than IDLE
        vault.expire(AGENT);
