...
contract SeatVaultV3Base is Test {
    ...
    function _params() internal pure returns (SeatVaultV3.Params memory) {
        return SeatVaultV3.Params({
    ...
    function _setUpVault(IERC20 tok) internal {
        vault = new SeatVaultV3(pool, tok, owner, sink, _params());
    ...
    function _seat(uint256 id) internal view returns (SeatVaultV3.Seat memory) {
        return vault.getSeat(id);
    }

    function _status(uint256 id) internal view returns (SeatVaultV3.Status) {
...
contract SeatVaultV3Test is SeatVaultV3Base {
    // ------------------------------------------------------------------
    // Identity and stake
    // ------------------------------------------------------------------

    function test_adopt_onceOnlyOwnedAndFresh() public {
        SeatVaultV3 v = new SeatVaultV3(pool, IERC20(address(priors)), owner, sink, _params());
        vm.prank(agentOp);
        uint256 other = reg.register("x");
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.NotOurs.selector, other));
        v.adopt(other);
        vm.prank(anyone);
        ...
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.NotOurs.selector, AGENT));
        v.adopt(AGENT);
        vm.prank(agentOp);
        ...
        v.adopt(other);
        vm.prank(owner);
        vm.expectRevert(SeatVaultV3.AlreadyAdopted.selector);
        v.adopt(other);
    }
    ...
    function test_fund_needsAdoptionAndMinStake() public {
        SeatVaultV3 v = new SeatVaultV3(pool, IERC20(address(priors)), owner, sink, _params());
        vm.expectRevert(SeatVaultV3.NotAdopted.selector);
        v.fund(10 * USDC);
        vm.startPrank(owner);
        ...
        v.fund(5 * USDC);
        vm.expectRevert(SeatVaultV3.ZeroAmount.selector);
        v.fund(0);
        vm.stopPrank();
    }
    ...
    function test_offer_escrowsExactlyOneSeatUnderCurrentTerms() public {
        ...
        vm.prank(staker);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.OfferExists.selector, AGENT, staker));
        vault.offer(AGENT);
    }
    ...
    function test_offer_refusesUnknownZeroSelfRootsAndDefaulted() public {
        ...
        vm.expectRevert();
        vault.offer(999);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.NotSeatable.selector, 0));
        vault.offer(0);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.NotSeatable.selector, VAULT_ID));
        vault.offer(VAULT_ID);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.NotSeatable.selector, ROOT));
        vault.offer(ROOT);
        vm.stopPrank();
        _offerAndAccept(staker, AGENT_PK, AGENT);
    ...
    function test_offer_refusesAFeeOnTransferToken() public {
        ...
        SeatVaultV3 v = new SeatVaultV3(pool, IERC20(address(tax)), owner, sink, _params());
        vm.startPrank(owner);
        ...
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.BadTransfer.selector, SEAT, SEAT - SEAT / 100));
        v.offer(AGENT);
        vm.stopPrank();
    }
    ...
    function test_withdrawOffer_stakerOrControllerOnly() public {
        ...
        assertEq(line, 0);
        vm.prank(staker);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.NoOffer.selector, AGENT, staker));
        vault.withdrawOffer(AGENT, staker);
    }
    ...
    function test_accept_onlyTheAgentsController() public {
        ...
        vm.prank(staker2);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.NotController.selector, AGENT, staker2));
    ...
    function test_seat_selfSeatInOneCall() public {
        ...
        vm.prank(anyone);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.NotController.selector, AGENT2, anyone));
    ...
    function test_accept_termsBoundToTheOffer() public {
        ...
        (CreditPoolV2.Consent memory c, bytes memory sig) = _consent(AGENT, AGENT_PK);
        vm.prank(agentOp);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.TermsChanged.selector, AGENT));
        vault.accept(AGENT, staker, c, sig);
        p = _params();
        p.seatSize = SEAT * 2;
        vm.prank(owner);
        vault.setParams(p);
        vm.prank(agentOp);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.OfferTooSmall.selector, AGENT, SEAT, SEAT * 2));
        vault.accept(AGENT, staker, c, sig);
    }
    ...
    function test_accept_epochCapBoundsNewLines() public {
        ...
        vm.prank(agentOp2);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.EpochCapReached.selector, 5 * USDC, 0));
    ...
    function test_accept_maxOpenSeats() public {
        ...
        (CreditPoolV2.Consent memory c, bytes memory sig) = _consent(AGENT2, AGENT2_PK);
        vm.prank(agentOp2);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.TooManySeats.selector, 1, 1));
        vault.accept(AGENT2, staker2, c, sig);
    }
    ...
    function test_registerAndSeat_respectsPauseAndCaps() public {
        ...
        vm.prank(staker);
        vm.expectRevert(SeatVaultV3.Paused.selector);
    ...
    function test_close_withALoanOut_closesWhenItIsRepaid() public {
        ...
        assertEq(uint8(_status(AGENT)), uint8(SeatVaultV3.Status.Closed), "closed in the repayment");
        assertEq(priors.balanceOf(staker), before + SEAT);
    ...
    /// V3 (audit V-2): the pause stops new seats AND new loans on seats already open; a loan open before the pause
    /// is repaid as usual, and closing, withdrawing and claiming keep working. (V2's version was
    /// test_seatsPaused_stopsNewSeatsOnly: it borrowed on the open seat while paused.)
    function test_seatsPaused_stopsNewSeatsAndNewLoans() public {
        _offerAndAccept(staker, AGENT_PK, AGENT);
        uint256 loan = _borrow(agentOp, AGENT, 5 * USDC, 30 days); // open before the pause
        vm.prank(staker2);
        vault.offer(AGENT2);
        vm.prank(owner);
        vault.pauseSeats(true);
        ...
        _repay(agentOp, loan); // repaying always works
        vm.expectRevert(abi.encodeWithSelector(CreditPoolV2.BorrowBlockedByBacker.selector, VAULT_ID));
        vm.prank(agentOp);
        pool.borrow(AGENT, 5 * USDC, 1 days, agentOp, type(uint256).max); // the pause reaches the open seat
        vm.prank(owner);
        vault.pauseSeats(false);
        _repay(agentOp, _borrow(agentOp, AGENT, 5 * USDC, 1 days)); // and lifting it restores the seat
        vm.prank(staker);
        vault.close(AGENT);
        vm.prank(staker);
        vault.claim(staker);
    ...
    function test_graduation_handoffToABiggerBacker() public {
        ...
        vm.prank(rootOp);
        pool.vouchWithConsent(ROOT, AGENT, 50 * USDC, 0, c, sig);
        assertEq(uint8(_status(AGENT)), uint8(SeatVaultV3.Status.Closed), "graduated");
    ...
    /// The owner cannot change a seat's terms after it opened: a loan open when the terms change settles by the
    /// opening terms. V3 (audit V-2): a repricing does reach the open seat, as a refusal of any NEW loan until the
    /// seat is reopened at the new terms. (V2's version borrowed after the repricing; V3 refuses that loan.)
    function test_default_termsFixedAtOpening() public {
        _offerAndAccept(staker, AGENT_PK, AGENT);
        uint256 loan = _borrow(agentOp, AGENT, 5 * USDC, 1 days); // open before the repricing
        SeatVaultV3.Params memory p = _params();
        p.burnBps = 10_000;
        vm.prank(owner);
        vault.setParams(p);
        // no new loan at the old terms (asked of the vault directly: the $5 line is fully drawn, so the pool would
        // stop a second borrow on capacity before consulting the vault)
        assertFalse(vault.canBorrow(VAULT_ID, AGENT, 5 * USDC, 1 days, 0, agentOp, agentOp, agentOp));
        uint256 before = priors.balanceOf(staker);
        _default(loan);
    ...
    function test_settle_afterAFailedOnDefault() public {
        ...
        vm.prank(staker);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.AgentDefaulted.selector, AGENT));
        vault.close(AGENT);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.BadProof.selector, AGENT, 0));
    ...
    function test_settle_afterAFailedGraduation_thenADefaultElsewhere() public {
        ...
        _handoff(AGENT, AGENT_PK, 20 * USDC);
        sw.setBlocked(false);
        assertEq(uint8(_status(AGENT)), uint8(SeatVaultV3.Status.Open), "stale: the hook failed");
    ...
    function test_settle_afterAFailedLeave_noDefault() public {
        SwitchPriors sw = _switchVault();
        _offerAndAccept(staker, AGENT_PK, AGENT);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.StillBacked.selector, AGENT));
        vault.settle(AGENT, 0);
        sw.setBlocked(true);
        vm.prank(agentOp);
        pool.leave(AGENT);
    ...
    function test_rescue_cannotTouchStakersTokensOrUSDG() public {
        _offerAndAccept(staker, AGENT_PK, AGENT);
        vm.startPrank(owner);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.Protected.selector, address(usdc)));
        vault.rescue(address(usdc), owner);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV3.Protected.selector, address(priors)));
    ...
    function test_constructor_refusesUSDGAsTheSeatToken() public {
        vm.expectRevert(SeatVaultV3.InvalidParams.selector);
        new SeatVaultV3(pool, IERC20(address(usdc)), owner, sink, _params());
    }
...
contract SeatVaultV3MigrationTest is SeatVaultV3Base {
    CreditPool v1;
