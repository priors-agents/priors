...
/// Independent review of SeatVaultV2 (2026-09-23). Each `test_F*` PASSES by demonstrating the finding.
contract ReviewSeatVaultV2 is SeatVaultV2Base {
    ...
    function test_F4_idleSeatsFillTheVaultAndOnlyTheSquatterCanFreeThem() public {
        ...
        vm.prank(owner);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV2.NotStakerOrController.selector, first, owner));
        vault.close(first);
        ...
        vm.prank(agentOp);
        vm.expectRevert(abi.encodeWithSelector(SeatVaultV2.TooManySeats.selector, 2, 2));
