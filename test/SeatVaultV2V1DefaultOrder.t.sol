...
///
/// `SeatVaultV2MigrationTest` pins one order: the agent was never imported, it defaults on v1 while seated, then
...
/// PoolV2Lib.v1Clean (blocks new v2 borrows while v1 shows a default), SeatVaultV2.onRelease / close.
contract SeatVaultV2V1DefaultOrderTest is SeatVaultV2Base {
    CreditPool v1;
    uint256 v1root;
    ...
    function test_A_importedBeforeTheV1Default_seatReturnsEveryToken() public {
        ...
        vm.prank(staker);
        vault.close(AGENT);
        assertEq(uint8(_status(AGENT)), uint8(SeatVaultV2.Status.Closed), "seat did not close");
