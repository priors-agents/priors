...
contract SeatV2Handler is Test {
    ...
    mapping(uint256 => SeatVaultV2.Status) public lastStatus;
    ...
    modifier track() {
        ...
        for (uint256 i = 0; i < agents.length; i++) {
            uint256 id = agents[i];
            SeatVaultV2.Seat memory s = vault.getSeat(id);
            SeatVaultV2.Status was = lastStatus[id];
            if (was == SeatVaultV2.Status.Open && s.status == SeatVaultV2.Status.Settled) {
                uint256 b = s.amount * s.burnBps / 10_000;
                burntFrom[s.staker] += b;
                expectedBurn += b;
                okSettles++;
            } else if (was == SeatVaultV2.Status.Open && s.status == SeatVaultV2.Status.Closed) {
                okCloses++;
            }
    ...
    function registerAndSeat(uint256 who) external track {
        ...
        try vault.registerAndSeat("") returns (uint256 id) {
            ...
            lastStatus[id] = SeatVaultV2.Status.Open;
            okOpens++;
            okRegisters++;
        } catch {}
    }
    ...
    function leave(uint256 a) external track {
        (uint256 id, uint256 pk) = _agent(a);
        SeatVaultV2.Seat memory s = vault.getSeat(id);
        bool seated = s.status == SeatVaultV2.Status.Open && pool.getAgent(id).sponsor == vaultId;
        ...
        try pool.leave(id) {
            if (seated) {
                okGraduations++;
                if (
                    vault.getSeat(id).status != SeatVaultV2.Status.Closed
    ...
    function borrow(uint256 a, uint256 amount, uint256 term) external track {
        ...
        if (vault.getSeat(id).status != SeatVaultV2.Status.Open) {
            (CreditPoolV2.Consent memory c, bytes memory sig) = _consent(id, vaultId, pk);
            vm.prank(vm.addr(pk));
            try vault.seat(id, c, sig) {
                _consumed(id, vm.addr(pk));
    ...
    function repay(uint256 i) external track {
        ...
        SeatVaultV2.Seat memory s = vault.getSeat(ln.agentId);
        bool credited = ln.sponsorId == vaultId && s.status == SeatVaultV2.Status.Open;
        vm.prank(actors[0]);
    ...
    function markDefault(uint256 i) external track {
        ...
        bool seated = ln.sponsorId == vaultId && vault.getSeat(ln.agentId).status == SeatVaultV2.Status.Open;
        try pool.markDefault(l) {
            okDefaults++;
            if (seated && vault.getSeat(ln.agentId).status != SeatVaultV2.Status.Settled) defaultNotSettled = true;
        } catch {}
    }
    ...
    function retune(uint256 burnBps, uint256 seatSize, uint256 line) external track {
        SeatVaultV2.Params memory p = SeatVaultV2.Params({
...
contract SeatVaultV2Invariants is Test {
    uint256 constant USDC = 1e6;
    SeatV2Handler h;
    SeatVaultV2 vault;
    CreditPoolV2 pool;
    ...
    function _escrow() internal view returns (uint256[] memory e, uint256 total) {
        ...
        for (uint256 i = 0; i < h.agentCount(); i++) {
            SeatVaultV2.Seat memory s = vault.getSeat(h.agents(i));
            if (s.status == SeatVaultV2.Status.Open) {
    ...
    function invariant_feesExact() public view {
        ...
        for (uint256 i = 0; i < h.agentCount(); i++) {
            ...
            SeatVaultV2.Seat memory s = vault.getSeat(id);
            if (s.status == SeatVaultV2.Status.Open) pending[_idx(s.staker)] += vault.pendingFees(id);
        }
        for (uint256 k = 0; k < n; k++) {
            address a = h.actors(k);
    ...
    function invariant_noLineWithoutASeat() public view {
        ...
        for (uint256 i = 0; i < h.agentCount(); i++) {
            ...
            bool seated = vault.getSeat(id).status == SeatVaultV2.Status.Open;
            if (seated) open++;
