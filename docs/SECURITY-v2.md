...
| AI-1 | Medium | **Self-service invites.** The Telegram invite bot has a self-service mode in which it signs a treasury invite for any owner who proves control of an agent, up to 10 a day. Agents and wallets are free, so while it is on one person can take the week's first lines. | **Off today:** invites are approved by an admin, then signed by the treasury's named inviter after an ownership check. **When on, bounded** by treasury v4's `epochCap` ($25 per 7-day epoch) and its free backing, paid from treasury stake; lenders are untouched. | internal review of the bot, 2026-09-25 |
