...
contract SeatVaultV2 is Ownable2Step, IERC721Receiver, IERC1271, IBackerHook {
    ...
    bytes32 internal constant LOCK = keccak256("priors.seatvault.v2.lock");
    bytes32 internal constant CONSENT = keccak256("priors.seatvault.v2.consent");
    ...
    function close(uint256 id) external nonReentrant {
        ...
        if (msg.sender != s.staker && !pool.isController(id, msg.sender)) revert NotStakerOrController(id, msg.sender);
        CreditPoolV2.Agent memory a = pool.getAgent(id);
    ...
    /// @notice Borrows are allowed only on an open seat that is not closing.
    function canBorrow(uint256 rootId, uint256 id, uint256, uint64, uint256, address, address, address)
        external
        view
        returns (bool)
    {
        Seat storage s = seats[id];
        return rootId == agentId && s.status == Status.Open && !s.closing;
    }
